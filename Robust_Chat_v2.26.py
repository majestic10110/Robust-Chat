...
class RobustChatSimpleV16:
    ...
    def _cat_qsy_mhz(self, mhz: float) -> bool:
        # CAT logic: Icom CI-V or Yaesu CAT based on self.cat_protocol
    ...
    def __init__(self, master: tk.Tk):
        self.master = master
        self.master.title("Robust Chat v2.26")
...
<head>
  ...
  <style>
    ...
    header { padding:10px 14px; background:#111b21; border-bottom:1px solid #202c33; display:flex; justify-content:space-between; align-items:center; }
    ...
    #chatlist, #locator { width:28%; min-width:120px; border-right:1px solid #202c33; overflow:auto; padding:8px; box-sizing:border-box; }
    ...
    #sendbar input[type=text] { padding:6px; border-radius:6px; border:1px solid #202c33; background:#0b141a; color:#e9edef; flex:1; }
    #sendbar button { padding:6px 12px; border-radius:6px; border:none; background:#0A84FF; color:#fff; font-weight:600; }
    #sendbar select { padding:6px; border-radius:6px; border:1px solid #202c33; background:#0b141a; color:#e9edef; }
  ...
  <script>
    ...
    function renderState() {
      ...
      keys.forEach(k => {
        ...
        div.className = 'call';
        div.onclick = () => {
          const sel = document.getElementById('dst');
          if (k === 'All' || k === 'ALL') {
            sel.value = 'ALL';
          } else {
            let opt = Array.from(sel.options).find(o => o.value === k);
            if (!opt) {
              opt = new Option(k, k);
              sel.add(opt);
            }
