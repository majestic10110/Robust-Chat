...
class RobustChatSimpleV16:
    ...
    def _cat_update_controls_state(self):
        ...
        try:
            ...
            self.auto_qsy_day_combo.configure(state=state if (connected and self.auto_qsy_enabled_var.get()) else ("disabled" if connected else "disabled"))
            self.auto_qsy_night_combo.configure(state=state if (connected and self.auto_qsy_enabled_var.get()) else ("disabled" if connected else "disabled"))
    ...
    def __init__(self, master: tk.Tk):
        self.master = master
        self.master.title("Robust Chat v2.20")
...
if __name__ == "__main__":
    ...
    _startup_log("===== Robust Chat v2.17 startup =====")
    try:
        root = main()
        _startup_log("Fonts and UI initialised OK")
        _startup_log("Entering mainloop()")
        root.mainloop()
        _startup_log("===== Robust Chat v2.17 shutdown =====")
    except Exception:
